...
def get_class_pk_lin(cosmo):
    ...
    if want_buba:
        ...
        for key in header.keys():
            if key in ['omega_b', 'omega_cdm', 'ztmp0', 'ztmp1', 'ztmp2', 'ztmp3', 'ztmp4', 'ztmp5', 'ztmp6', 'theta_s_100']:
                pass
            else:
                params[key] = header[key]

        fid_A_s = params.pop('A_s')
        try:
            A_s = (cosmo['sigma8']/params['sigma8_cb'])**2*fid_A_s
            cosmo['A_s'] = A_s
        except:
            pass
        params.pop('sigma8_cb')
        params['A_s'] = cosmo['A_s']
        print(params.items())
        params['h'] = cosmo['h']
        params['Omega_cdm'] = cosmo['Omega_c']
        ...
        params['z_max_pk'] = 1.0/cosmo.cosmo.spline_params.A_SPLINE_MINLOG_PK-1.0
